...
const WinterScene: React.FC<{ theme: Theme; gesture: GestureType }> = ({ theme, gesture }) => {
  ...
  useEffect(() => {
    ...
    if (treeRef.current && typeof gsap !== 'undefined') {
      ...
      for (let i = 0; i < P_COUNT; i++) {
        gsap.to(ca.array, { [i * 3]: tc.r, [i * 3 + 1]: tc.g, [i * 3 + 2]: tc.b, duration: 1.2, onUpdate: () => { ca.needsUpdate = true; } });
      }
    }
  }, [theme]);
